...
#'
#' Returns historical data from cash flow statements.
#'
#' @param ticker The stock ticker of the company as a string.
#' @param format Choose between "raw", "clean" and "tidy" format. The "raw" format returns the cash flow statement as you see it on yahoo finance, and is the easiest to read. "clean" keeps the same format, but removes empty grouping rows, and converts missing data to NA and numbers to numeric. "tidy" makes every item a varaible and every yearly report an observation, so that tidyverse packages can be easily used on the data. Default is "tidy".
